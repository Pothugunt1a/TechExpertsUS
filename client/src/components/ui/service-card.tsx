...
export function ServiceCard({ icon: Icon, title, description, link, index = 0, image, logo }: ServiceCardProps) {
  ...
  const card = (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 bg-black/60 dark:bg-black/40 backdrop-blur-sm flex flex-col justify-between p-8"
        initial={{ y: "100%" }}
        animate={{ 
          y: isHovered ? "0%" : "100%"
        }}
      ...
      >
        ...
        >
          ...
          <p className="text-white/90 dark:text-gray-300 mb-6 leading-relaxed">
            {description}
          </p>
          {link && (
            <motion.div
